...
import { setInstructorLoggedIn } from "../redux/slices/instructorDataSlice";
...
function InstructorLogin() {
  ...
  const onSubmit = async (data) => {
    if (userLoggedIn) {
      return toast.warning("Logout out from different role to get started...");
    }
    try {
      ...
      if (result.data.user.role != "instructor") {
        return toast.error("Not Valid Instructor Credential");
      }
      dispatch(setInstructorLoggedIn(result.data));
      ...
      <div className="max-w-md w-full space-y-8">
        <div>
          <h2 className="mt-6 text-center text-3xl font-bold text-gray-900">
            Login to Learnify as{" "}
            <span className="text-blue-500">Instructor</span>
          </h2>
        </div>
        <form
